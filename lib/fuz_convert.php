    ...
    if (file_exists($fuzFileName)) {
        ...
        if (!file_exists($sOutputXwm) || true) {
            ...
            if ($fuzMagic === 'FUZE') {
				...
				$command = "ffmpeg -v 0 -y -f xwma -i $tmpXwmFile -ar 22050 -ac 1 -sample_fmt s16 $sOutputXwm &>/dev/null";
...
function xwmToWav($fuzFileName)
{
    if (file_exists($fuzFileName)) {
        ...
        // Extract the xwm data from the fuz data file
        if (!file_exists($sOutputXwm) || true) {
          	$command = "ffmpeg -v 0 -y -f xwma -i $fuzFileName -ar 22050 -ac 1 -sample_fmt s16 $sOutputXwm &>/dev/null";
...
function wavToWav($fuzFileName)
{
    if (file_exists($fuzFileName)) {
        ...
        // Extract the xwm data from the fuz data file
        if (!file_exists($sOutputXwm) || true) {
          	$command = "ffmpeg -v 0 -y -f wav -i $fuzFileName -ar 22050 -ac 1 -sample_fmt s16 $sOutputXwm &>/dev/null";
