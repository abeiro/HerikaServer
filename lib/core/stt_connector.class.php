...
class STTConnector
{
    private $table = "core_stt_connector";
    ...
    public function getDriverOptions(): array
    {
        ...
        $values = $schema['STTFUNCTION']['values'] ?? [];
    ...
    public function getProviderFieldSchema($driver): array
    {
        ...
        $schema = $this->loadRawSchema();
        $providerSchema = $schema['STT'][$providerKey] ?? [];
    ...
    {
        $providerKey = $this->getProviderKeyFromDriver($driver);
        return $providerKey !== '' && isset(self::$apiBadgeLabelMap[$providerKey]);
    }
    ...
    public function setOldGlobals($currentConnectorData)
    {
        ...
        if ($driver === '') {
            $GLOBALS["STTFUNCTION"] = 'none';
            return;
        }

        $providerKey = $this->getProviderKeyFromDriver($driver);
        $GLOBALS["STTFUNCTION"] = $driver;
        if (!isset($GLOBALS["STT"]) || !is_array($GLOBALS["STT"])) {
            $GLOBALS["STT"] = [];
        }
        if ($providerKey !== '') {
            $GLOBALS["STT"][$providerKey] = [];
        }
        ...
        if ($providerKey !== '') {
            foreach ($metadata as $key => $value) {
                ...
                $GLOBALS["STT"][$providerKey][$key] = $value;
            }
        }
    }
