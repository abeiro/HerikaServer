...
if (!function_exists('quest_asset_export_manifest')) {
    function quest_asset_export_manifest($packKey)
    {
        if (!quest_asset_library_tables_ready()) {
            return null;
        }

        $packKey = quest_asset_normalize_key($packKey);
        if ($packKey === null) {
            return null;
        }
        $packKeyCn = $GLOBALS['db']->escape($packKey);
        $pack = $GLOBALS['db']->fetchOne("
            SELECT pack_key, label, game, manifest_version, required_plugins_json,
                   source, note, active
            FROM public.quest_asset_packs
            WHERE pack_key = '{$packKeyCn}'
            LIMIT 1
        ");
        if (!is_array($pack) || empty($pack['pack_key'])) {
            return null;
        }

        $assetRows = $GLOBALS['db']->fetchAll("
            SELECT stable_ref, signature, editor_id, display_name, source_plugin,
                   winning_plugin, metadata_json, safety_status, active
            FROM public.quest_assets
            WHERE source_pack = '{$packKeyCn}'
            ORDER BY source_plugin, signature, editor_id, stable_ref
        ");
        $groupRows = $GLOBALS['db']->fetchAll("
            SELECT dataset_name, group_key, label, description, selection_policy_json, active
            FROM public.quest_asset_groups
            WHERE source_pack = '{$packKeyCn}'
            ORDER BY dataset_name, group_key
        ");
        $memberRows = $GLOBALS['db']->fetchAll("
            SELECT dataset_name, group_key, stable_ref, weight, constraints_json, note, active
            FROM public.quest_asset_group_members
            WHERE source_pack = '{$packKeyCn}'
            ORDER BY dataset_name, group_key, stable_ref
        ");

        $membersByGroup = [];
        foreach ($memberRows as $row) {
            $identity = ($row['dataset_name'] ?? '') . '|' . ($row['group_key'] ?? '');
            $membersByGroup[$identity][] = [
                'stable_ref' => (string) ($row['stable_ref'] ?? ''),
                'weight' => intval($row['weight'] ?? 1),
                'constraints' => json_decode((string) ($row['constraints_json'] ?? '{}'), true) ?: [],
                'note' => (string) ($row['note'] ?? ''),
                'active' => quest_asset_db_bool($row['active'] ?? false),
            ];
        }
        ...
        foreach ($groupRows as $row) {
            ...
            $groups[] = [
                ...
                'selection_policy' => json_decode((string) ($row['selection_policy_json'] ?? '{}'), true) ?: [],
                'active' => quest_asset_db_bool($row['active'] ?? false),
                'members' => $membersByGroup[$identity] ?? [],
            ];
        }
        ...
        foreach ($assetRows as $row) {
            $assets[] = [
                ...
                'winning_plugin' => (string) ($row['winning_plugin'] ?? ''),
                'metadata' => json_decode((string) ($row['metadata_json'] ?? '{}'), true) ?: [],
                'safety_status' => (string) ($row['safety_status'] ?? 'review'),
                'active' => quest_asset_db_bool($row['active'] ?? false),
            ];
        }
        ...
        return [
            ...
            'pack' => [
                ...
                'game' => (string) $pack['game'],
                'version' => (string) $pack['manifest_version'],
                'required_plugins' => json_decode((string) $pack['required_plugins_json'], true) ?: [],
                'source' => (string) $pack['source'],
                'note' => (string) $pack['note'],
                'active' => quest_asset_db_bool($pack['active'] ?? false),
            ],
            'assets' => $assets,
            'groups' => $groups,
        ];
