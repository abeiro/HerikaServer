...
$currentConf = array(
    "PLAYER_NAME" => array(
        ...
        "_title" => "QUICK START MENU",
        "currentValue" => ''
    ),
    "openrouter API_KEY" => array(
        "type" => "apikey",
        "description" => "Copy and Paste your OpenRouter API Key <br><a href='https://openrouter.ai/' target='_blank'>SETUP ACCOUNT HERE</a> <b>YOU MUST PUT AT LEAST $5 ON IT!</b>",
        "code" => "OPENAI_API_KEY",
        "currentValue" => ''
    ),
    "openrouterjson API_KEY" => array(
        "type" => "apikey",
        "description" => "Copy and Paste your OpenRouter API Key. <i>Yes we need to do it 2 times.</i> <br><a href='https://openrouter.ai/' target='_blank'>SETUP ACCOUNT HERE</a> <b>YOU MUST PUT AT LEAST $5 ON IT!</b>",
        "code" => "OPENAI_API_KEY",
        "currentValue" => ''
    ),
    "TTSFUNCTION" => array(
        "type" => "select",
        "values" => ["melotts","xtts-fastapi"],
        "description" => "Select either MeloTTS or XTTS if you installed it under Tools/Components/NVIDIA GPU. Recommend to use MeloTTS for most users.<br>Make sure you have installed the voice files!",
        "_title" => "Text-to-Speech Service",
        "currentValue" => ''
    ),
    "STT WHISPER API_KEY" => array(
        "type" => "apikey",
        "description" => "Copy and Paste your OpenAI API Key <br><a href='https://platform.openai.com/docs/overview/' target='_blank'>SETUP ACCOUNT HERE</a> <b>YOU MUST PUT AT LEAST $5 ON IT!</b>",
        "code" => "OPENAI_API_KEY",
        "currentValue" => ''
    )
    // Removed openrouter and openrouterjson other configurations
);
...
foreach ($currentConf as $pname => $parms) {
    ...
    if ($parms["type"] == "string") {
        ...
        echo "<input type='url' class='form-control' id='$fieldName' name='".htmlspecialchars($fieldName)."' value='".htmlspecialchars($parms["currentValue"])."' $FORCE_DISABLED>";
        ...
        echo "<input type='number' class='form-control' id='$fieldName' name='".htmlspecialchars($fieldName)."' value='".htmlspecialchars($parms["currentValue"])."' step='1' $FORCE_DISABLED>";
    } 
    else if ($parms["type"] == "number") {
        echo "<input type='number' class='form-control' id='$fieldName' name='".htmlspecialchars($fieldName)."' value='".htmlspecialchars($parms["currentValue"])."' step='0.01' $FORCE_DISABLED>";
    } 
    else if ($parms["type"] == "apikey") {
        $jsid = strtr($fieldName, ["@" => "_"]);
        echo "<div class='input-group'>";
        echo "<input type='password' class='form-control' id='$jsid' name='".htmlspecialchars($fieldName)."' value='".htmlspecialchars($parms["currentValue"])."' style='filter: blur(3px);' $FORCE_DISABLED>";
        echo "<div class='input-group-append'>";
    ...
    // For example, 'util', 'ormodellist', etc.
...
<div class="btn-group-custom text-center">
        <p class="warning-text">
    After you click save we <b>HIGHLY RECOMMEND</b> to run the Troubleshooting tests (in the menu above) for the LLM/AI, TTS and STT to verify everything is setup correctly.
</p>
