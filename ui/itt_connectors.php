...
{
    $activeId = chimGetGeneralSettingInt('GLOBAL_ITT_CONNECTOR_ID', 0);
    if ($activeId > 0) {
    ...
    if ($migrated && !empty($migrated['id'])) {
        ...
        chimSetGeneralSetting('GLOBAL_ITT_CONNECTOR_ID', $activeId, chimGetSchemaDescription('GLOBAL_ITT_CONNECTOR_ID'));
        return $activeId;
    }
    ...
    if ($createdId > 0) {
        chimSetGeneralSetting('GLOBAL_ITT_CONNECTOR_ID', $createdId, chimGetSchemaDescription('GLOBAL_ITT_CONNECTOR_ID'));
    }
    return $createdId;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_connector'])) {
    ...
    chimSetGeneralSetting('GLOBAL_ITT_CONNECTOR_ID', $savedId, chimGetSchemaDescription('GLOBAL_ITT_CONNECTOR_ID'));
...
$currentDriver = $connector->normalizeDriverValue($editingRow['driver'] ?? 'openrouter');
if ($currentDriver === '') {
    $currentDriver = 'openrouter';
}
...
.btn-row .btn-save, .btn-row .btn-primary { margin: 0; }
.btn-save, .btn-primary { border-radius: 8px; padding: 8px 14px; cursor: pointer; border: 1px solid rgba(138,155,182,.35); text-decoration: none; display: inline-flex; align-items: center; gap: 6px; }
...
#itt_test_modal { position: fixed; inset: 0; display: none; align-items: center; justify-content: center; background: rgba(0,0,0,.66); z-index: 9999; }
#itt_test_modal .inner { width: min(1100px, 94vw); height: min(820px, 92vh); background: #0f1724; border: 1px solid #3a3a3a; border-radius: 10px; position: relative; overflow: hidden; }
#itt_test_modal iframe { width: 100%; height: 100%; border: 0; background: #111; }
        ...
        <div class="layout">
            <div class="left-col">
                <div class="summary-note">
                    This page edits the single global ITT connector. It controls which vision-capable backend HerikaServer uses for screenshots and image analysis.
                </div>
                                ...
                                <select id="driver" name="driver">
                                    <?php foreach ($groupedDriverOptions as $groupLabel => $groupDrivers): ?>
                                        ...
                                        <optgroup label="<?php echo ih($groupLabel); ?>">
                                            <?php foreach ($groupDrivers as $driverValue): ?>
                                                <option value="<?php echo ih($driverValue); ?>" <?php echo $currentDriver === $driverValue ? 'selected' : ''; ?>>
                                                    <?php echo ih($connector->getDisplayName($driverValue)); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </optgroup>
                                    <?php endforeach; ?>
                                </select>
                                <div class="field-help">Choose the image-to-text backend HerikaServer should load globally.</div>
                            </div>
                            ...
                            <div class="field-block" id="api_badge_block" style="<?php echo $connector->driverUsesApiBadge($currentDriver) ? '' : 'display:none;'; ?>">
                                ...
                                <select id="api_badge_id" name="api_badge_id">
                                    ...
                                    <?php foreach ($withKey as $apiRow): ?>
                                        ...
                                        ?>
                                        <option value="<?php echo ih($apiRowId); ?>" data-empty="0" <?php echo $selected; ?>>
                                            <?php echo ih($labelText); ?>
                                        </option>
                                    <?php endforeach; ?>
                                    <?php if (!empty($noKey)): ?>
                                        ...
                                        <?php foreach ($noKey as $apiRow): ?>
                                            ...
                                            ?>
                                            <option value="<?php echo ih($apiRowId); ?>" data-empty="1" <?php echo $selected; ?>>
                                                <?php echo ih($labelText); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                        ...
                                        <?php foreach ($groupSchema as $fieldName => $definition): ?>
                                            ...
                                            <div class="field-block">
                                                ...
                                                <?php if ($fieldType === 'boolean'): ?>
                                                    <input type="hidden" name="<?php echo ih($fieldKey); ?>" value="false">
                                                ...
                                                <?php elseif ($fieldType === 'select'): ?>
                                                    <select id="<?php echo ih($fieldKey); ?>" name="<?php echo ih($fieldKey); ?>">
                                                        <?php foreach (($definition['values'] ?? []) as $valueOption): ?>
                                                            <option value="<?php echo ih($valueOption); ?>" <?php echo strval($fieldValue) === strval($valueOption) ? 'selected' : ''; ?>>
                                                                <?php echo ih($valueOption); ?>
                                                            </option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                <?php elseif ($fieldType === 'selectmultiple'): ?>
                                                    <select id="<?php echo ih($fieldKey); ?>" name="<?php echo ih($fieldKey); ?>[]" multiple>
                                                        ...
                                                        <?php foreach (($definition['values'] ?? []) as $valueOption): ?>
                                                            <option value="<?php echo ih($valueOption); ?>" <?php echo in_array($valueOption, $selectedValues, true) ? 'selected' : ''; ?>>
                                                                <?php echo ih($valueOption); ?>
                                                            </option>
                                                        <?php endforeach; ?>
                                                    </select>
...
(function(){
    ...
    function syncDriverFields() {
        ...
        if (urlBlock) {
            ...
            if (urlInput) {
                ...
                if (currentValue === '' || currentValue === previousDefault) {
                    if (selected === 'custom' && nextDefault === '') {
                        urlInput.value = '';
                    } else if (nextDefault) {
        ...
        if (customNote) {
            customNote.style.display = selected === 'custom' ? '' : 'none';
        }
    ...
    async function saveBeforeTest() {
        ...
        try {
            await fetch(form.getAttribute('action') || 'itt_connectors.php', { method: 'POST', body: fd });
            return true;
        } catch (_error) {
            return false;
        }
    ...
    function openTestModal() {
        ...
        iframe.src = <?php echo json_encode($webRoot . '/ui/tests/itt-test.php'); ?> + '?cb=' + Date.now() + <?php echo json_encode($isEmbed ? '&embed=1' : ''); ?>;
