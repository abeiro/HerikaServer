...
if ($method === "POST") {
    ...
    if (!isset($GLOBALS["CONNECTORS_DIARY"]) || !file_exists($enginePath . "connector" . DIRECTORY_SEPARATOR . "{$GLOBALS["CONNECTORS_DIARY"]}.php")) {
        ...
        // Get max tokens for goals update
        $maxTokens = 800;
        ...
        $head = [
            ["role" => "system", "content" => "You are an assistant. Analyze the dialogue history and character profile to update ONLY the goals and aspirations for the character named '{$jsonDataInput["HERIKA_NAME"]}'. Focus exclusively on information about {$jsonDataInput["HERIKA_NAME"]} and ignore details about other characters mentioned in the dialogue."]
        ];

        $prompt = [
            ["role" => "user", "content" => "* Dialogue history:\n" . $historyData . ReplacePlayerNamePlaceholder($profileContextString)],
            ["role" => "user", "content" => "Character name: " . $jsonDataInput["HERIKA_NAME"] . "\nCurrent Goals:\n" . ReplacePlayerNamePlaceholder($currentGoals)],
        ...
        if (!empty($buffer)) {
            ...
            $FOLLOWER_CONF["HERIKA_GOALS"]=$buffer;
            ...
            */
            ...
            // Update or add HERIKA_GOALS variable
            $pattern = '/\$HERIKA_GOALS\s*=\s*[^;]+;/';
            if (preg_match($pattern, $content)) {
                $content = preg_replace($pattern, '$HERIKA_GOALS=' . $escapedValue . ';', $content);
            } else {
                $content = str_replace('?>', '$HERIKA_GOALS=' . $escapedValue . ';' . PHP_EOL . '?>', $content);
            }
            ...
            if (file_put_contents($profile, $content, LOCK_EX)) {
                echo json_encode([
                    ...
                    "updated_field" => "HERIKA_GOALS",
                    "new_value" => $buffer
                ]);
            } else {
                echo json_encode(["status" => "error", "message" => "Failed to save goals update to profile"]);
            }
        } else {
            echo json_encode(["status" => "error", "message" => "No goals update generated"]);
        }
    }
} else {
