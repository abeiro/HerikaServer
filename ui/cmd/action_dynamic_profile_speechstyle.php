...
if ($method === "POST") {
    ...
    if (!isset($GLOBALS["CONNECTORS_DIARY"]) || !file_exists($enginePath . "connector" . DIRECTORY_SEPARATOR . "{$GLOBALS["CONNECTORS_DIARY"]}.php")) {
        ...
        // Get max tokens for speechstyle update
        $maxTokens = 800;
        ...
        // Get current speechstyle value and prompt
        $currentSpeechstyle = isset($jsonDataInput["HERIKA_SPEECHSTYLE"]) ? $jsonDataInput["HERIKA_SPEECHSTYLE"] : '';
        $updatePrompt = isset($GLOBALS["DYNAMIC_PROMPT_SPEECHSTYLE"]) ? $GLOBALS["DYNAMIC_PROMPT_SPEECHSTYLE"] : '';

        if (empty($updatePrompt)) {
            echo json_encode(["status" => "error", "message" => "DYNAMIC_PROMPT_SPEECHSTYLE not configured"]);
            exit;
        }
        ...
        // Build prompt for speechstyle update
        $head = [
            ["role" => "system", "content" => "You are an assistant. Analyze the dialogue history and character profile to update ONLY the speech style for the character named '{$jsonDataInput["HERIKA_NAME"]}'. Focus mostly on information about {$jsonDataInput["HERIKA_NAME"]} and ignore details about other characters mentioned in the dialogue."]
        ];
        ...
        if (!empty($buffer)) {
            ...
            $FOLLOWER_CONF["HERIKA_SPEECHSTYLE"]=$buffer;
            ...
            */
            ...
            // Update or add HERIKA_SPEECHSTYLE variable
            $pattern = '/\$HERIKA_SPEECHSTYLE\s*=\s*[^;]+;/';
            if (preg_match($pattern, $content)) {
                $content = preg_replace($pattern, '$HERIKA_SPEECHSTYLE=' . $escapedValue . ';', $content);
            } else {
                $content = str_replace('?>', '$HERIKA_SPEECHSTYLE=' . $escapedValue . ';' . PHP_EOL . '?>', $content);
            }
            
            if (file_put_contents($profile, $content, LOCK_EX)) {
                echo json_encode([
                    "status" => "success", 
                    "message" => "Speech Style updated successfully!",
                    "updated_field" => "HERIKA_SPEECHSTYLE",
                    "new_value" => $buffer
                ]);
            } else {
                echo json_encode(["status" => "error", "message" => "Failed to save speech style update to profile"]);
            }
        } else {
            echo json_encode(["status" => "error", "message" => "No speech style update generated"]);
        }
    }
} else {
