...
if ($method === "POST") {
    ...
    if (!isset($GLOBALS["CONNECTORS_DIARY"]) || !file_exists($enginePath . "connector" . DIRECTORY_SEPARATOR . "{$GLOBALS["CONNECTORS_DIARY"]}.php")) {
        ...
        // Get max tokens for occupation update
        $maxTokens = 800;
        ...
        // Get current occupation value and prompt
        $currentOccupation = isset($jsonDataInput["HERIKA_OCCUPATION"]) ? $jsonDataInput["HERIKA_OCCUPATION"] : '';
        $updatePrompt = isset($GLOBALS["DYNAMIC_PROMPT_OCCUPATION"]) ? $GLOBALS["DYNAMIC_PROMPT_OCCUPATION"] : '';

        if (empty($updatePrompt)) {
            echo json_encode(["status" => "error", "message" => "DYNAMIC_PROMPT_OCCUPATION not configured"]);
            exit;
        }
        ...
        if (!empty($buffer)) {
            ...
            $FOLLOWER_CONF["HERIKA_OCCUPATION"]=$buffer;
            /*
            // Save directly to profile file
            $content = file_get_contents($profile);
            $escapedValue = var_export($buffer, true);
            
            // Update or add HERIKA_OCCUPATION variable
            $pattern = '/\$HERIKA_OCCUPATION\s*=\s*[^;]+;/';
            if (preg_match($pattern, $content)) {
                $content = preg_replace($pattern, '$HERIKA_OCCUPATION=' . $escapedValue . ';', $content);
            } else {
                $content = str_replace('?>', '$HERIKA_OCCUPATION=' . $escapedValue . ';' . PHP_EOL . '?>', $content);
            }
            ...
            */
            if (write_php_assignments($FOLLOWER_CONF,$profile)) {
                echo json_encode([
                    ...
                    "updated_field" => "HERIKA_OCCUPATION",
                    "new_value" => $buffer
                ]);
            } else {
                echo json_encode(["status" => "error", "message" => "Failed to save occupation update to profile"]);
            }
        } else {
            echo json_encode(["status" => "error", "message" => "No occupation update generated"]);
        }
    }
} else {
