...
if ($method === "POST") {
    ...
    if (!isset($GLOBALS["CONNECTORS_DIARY"]) || !file_exists($enginePath . "connector" . DIRECTORY_SEPARATOR . "{$GLOBALS["CONNECTORS_DIARY"]}.php")) {
        ...
        // Get max tokens for relationships update
        $maxTokens = 800;
        ...
        $head = [
            ["role" => "system", "content" => "You are an assistant. Analyze the dialogue history and update the character's relationships based on the information provided."]
        ];
        ...
        if (!empty($buffer)) {
            ...
            if (file_put_contents($profile, $content, LOCK_EX)) {
                ...
                echo json_encode(["status" => "error", "message" => "Failed to save relationships update to profile"]);
            }
        } else {
            echo json_encode(["status" => "error", "message" => "No relationships update generated"]);
        }
    }
} else {
