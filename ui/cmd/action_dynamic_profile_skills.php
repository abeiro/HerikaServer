...
if ($method === "POST") {
    ...
    if (!isset($GLOBALS["CONNECTORS_DIARY"]) || !file_exists($enginePath . "connector" . DIRECTORY_SEPARATOR . "{$GLOBALS["CONNECTORS_DIARY"]}.php")) {
        ...
        $head = [
            ["role" => "system", "content" => "You are an assistant. Analyze the dialogue history and update the character's skills and abilities based on the information provided."]
        ];
        ...
        if (!empty($buffer)) {
            ...
            if (file_put_contents($profile, $content, LOCK_EX)) {
                echo json_encode([
                    ...
                    "updated_field" => "HERIKA_SKILLS",
                    "new_value" => $buffer
                ]);
            } else {
                echo json_encode(["status" => "error", "message" => "Failed to save skills update to profile"]);
            }
        } else {
            echo json_encode(["status" => "error", "message" => "No skills update generated"]);
        }
    }
} else {
