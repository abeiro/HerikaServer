...
WITH expanded AS (
    SELECT
        location_key AS key_name,
        active,
        CASE WHEN formid >= 0 THEN formid ELSE NULL END AS formid
    FROM public.quest_master_data_locations
    UNION ALL
    SELECT
        q.location_key AS key_name,
        q.active,
        CASE WHEN j.value ~ '^-?\d+$' THEN j.value::bigint ELSE NULL END AS formid
    FROM public.quest_master_data_locations q
...
deleted AS (
    DELETE FROM public.quest_master_data_locations
)
INSERT INTO public.quest_master_data_locations (location_key, formid, formids_json, active, note)
SELECT
    key_name,
    -1,
...
WITH expanded AS (
    SELECT
        ...
        active,
        CASE WHEN formid >= 0 THEN formid ELSE NULL END AS formid
    FROM public.quest_item_locations
    UNION ALL
    SELECT
        q.location_key AS key_name,
        q.active,
        CASE WHEN j.value ~ '^-?\d+$' THEN j.value::bigint ELSE NULL END AS formid
    FROM public.quest_item_locations q
    CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(q.formids_json, '[]'::jsonb)) AS j(value)
),
normalized AS (
    SELECT
...
deleted AS (
    DELETE FROM public.quest_item_locations
)
INSERT INTO public.quest_item_locations (location_key, formid, formids_json, active, note)
SELECT
    key_name,
    -1,
    COALESCE(formids_json, '[]'::jsonb),
