...
if ($gameRequest[0] == "funcret") { // Take out the functions part
	...
	*/
    ...
    require_once(__DIR__ . "/../lib/core/book_context.php");
    ...
    $request = $bookPrompt;
    $GLOBALS['CHIM_ACTIVE_BOOK_CONTEXT'] = chimResolveGroundedBook(
        $db,
        (int)($gameRequest[2] ?? 0),
        1500
    );
    $bookTitle = chimBookNormalizeText($GLOBALS['CHIM_ACTIVE_BOOK_CONTEXT']['title'] ?? 'the open book');
    // Override player request here. This request is generated by the game plugin, in English.
    $gameRequest[3] = $PROMPTS["book"]["player_request"][0] . "<" . $bookTitle . ">";
    if (empty($GLOBALS['BOOK_EVENT_FULL'])) {
        $gameRequest[3] .= ' The book text is not being shared. Do not invent or summarize its contents.';
    } elseif (empty($GLOBALS['CHIM_ACTIVE_BOOK_CONTEXT']['available'])) {
        $gameRequest[3] .= ' The captured text is not available yet. Do not invent or summarize its contents.';
    }
